...
| `lune-assets`        | Embedded-asset HTTP server on macOS / Linux | Isolated accept loop on top of a 2-thread `lune-assets-pool` `Parallel` pool for per-connection request handling. On Win32, AssetServer spawns bind+listen via `::spawn` on the default context (same IOCP-affinity reason as Stream — separating the two contexts parks accept completions forever).                                                                                                                                                                                 |
