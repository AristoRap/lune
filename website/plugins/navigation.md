...
Injects a tiny shim that listens for `popstate` and `hashchange` and patches `history.pushState` / `replaceState` so SPA-router transitions also fire. The shim is only installed when `opts.on_navigate` is set — without a callback, the plugin does nothing at runtime.
