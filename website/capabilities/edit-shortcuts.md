...
A passive capability — injects a single `keydown` listener that maps the standard edit-action shortcuts (Select All, Copy, Paste, Cut, Undo, Redo) to `document.execCommand`. Nothing to configure; opt out via `lune.yml` if your app handles these keys itself.
