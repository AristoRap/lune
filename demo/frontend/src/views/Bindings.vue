...
async function callGreet() {
  greetOut.value = await api.Demo.greet({ name: greetIn.value });
}

async function callReverse() {
  revOut.value = await api.Demo.reverse({ text: revIn.value });
}

async function callFileInfo() {
  const raw = await api.Demo.fileInfo({ path: fiIn.value });
  try {
...
async function callFailWith() {
  errOut.value = null;
  try {
    await api.Demo.failWith({ code: errCode.value });
  } catch (err) {
    errOut.value = {
