...
- [ ] **Adopt [vow](https://github.com/AristoRap/vow) as the typed-RPC core** — lune's Crystal↔JS bridge (binding + dispatch + client codegen) was extracted into the standalone, transport-agnostic `vow` library; fold it back in phase by phase so lune consumes its own core. Phases 0–2 — share vow's strict type mapper, manifest-driven transitive type capture (structs auto-captured as named interfaces), then dispatch on `Vow::Registry` behind a webview transport (the generated client moved to vow's named-object calling convention) — have landed. Remaining: retiring lune's duplicated binding/generator glue once parity holds (Phase 3). vow stays standalone — every gap is absorbed lune-side or added to vow as a generic feature.
