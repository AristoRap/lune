...
- [ ] **Adopt [vow](https://github.com/AristoRap/vow) as the typed-RPC core** — lune's Crystal↔JS bridge (binding + dispatch + client codegen) was extracted into the standalone, transport-agnostic `vow` library; fold it back in phase by phase so lune consumes its own core. Phases 0–1 — share vow's strict type mapper, then manifest-driven transitive type capture (structs auto-captured as named interfaces) — have landed. Remaining: dispatch on `Vow::Registry` behind a webview transport (moving the generated client to vow's named-object calling convention), and retiring lune's duplicated binding/generator once parity holds. vow stays standalone — every gap is absorbed lune-side or added to vow as a generic feature.
