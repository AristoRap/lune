...
- [ ] Linux/Windows `window_drag` — `Plugins::WindowDrag` is darwin-only because the native shim is Cocoa-only; `opts.drag.zone` silently no-ops elsewhere. Win32: `ReleaseCapture()` + `SendMessage(hwnd, WM_NCLBUTTONDOWN, HTCAPTION, 0)`. X11: `XSendEvent` with `_NET_WM_MOVERESIZE`. Wayland: `xdg_toplevel.move()` via wl_seat.
