...
- [x] **Adopt [vow](https://github.com/AristoRap/vow) as the typed-RPC core** — lune's Crystal↔JS bridge (binding + dispatch + client codegen) was extracted into the standalone, transport-agnostic `vow` library and folded back in so lune consumes its own core. Shipped: vow's strict type mapper; manifest-driven transitive type capture (structs → named interfaces, enums → string-literal unions from their serialized form); dispatch on `Vow::Registry` behind a webview transport with vow's named-object calling convention; `@[Lune::Bind]` kept as a thin layer over vow's marker-driven dispatch (no duplicated decode/invoke/encode glue); `Lune::WindowContext < Vow::Context` for per-call window identity; error hints forwarded onto the wire; manifest introspection surfaced via `lune doctor api` (contract table or `--json`) and an `Introspection` plugin (`lune.Introspection.manifest()` + a devtools-only `window.__lune.manifest()` wrapper). vow stays standalone — every gap was absorbed lune-side or added to vow as a generic feature (e.g. enum capture and "did you mean?" diagnostics, which benefit vow's HTTP transports).
