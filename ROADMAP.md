...
- [ ] Linux/Windows `window_drag` — `Capabilities::WindowDrag` is darwin-only because the native shim is Cocoa-only; `opts.drag.zone` silently no-ops elsewhere. Win32: `ReleaseCapture()` + `SendMessage(hwnd, WM_NCLBUTTONDOWN, HTCAPTION, 0)`. X11: `XSendEvent` with `_NET_WM_MOVERESIZE`. Wayland: `xdg_toplevel.move()` via wl_seat.
...
- [ ] Window-aware option callbacks — `opts.on_navigate` (and likely `on_load` / `on_close`) gets only the URL today; in multi-window apps it can't tell which window fired. Extend the proc signature with a window id, or pass an event struct. Likely shares design with per-window capabilities.
