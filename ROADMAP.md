...
- [ ] Linux/Windows window drag — `opts.window.drag_zone` is darwin-only because the native shim is Cocoa-only; setting it elsewhere installs no listener. Win32: `ReleaseCapture()` + `SendMessage(hwnd, WM_NCLBUTTONDOWN, HTCAPTION, 0)`. X11: `XSendEvent` with `_NET_WM_MOVERESIZE`. Wayland: `xdg_toplevel.move()` via wl_seat.
