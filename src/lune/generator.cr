...
module Lune
  module Generator
    ...
    def self.generate_runtime_dts(
      ...
      ts_type_block = render_registered_ts_types
    ...
    # Renders every `Lune.register_ts_type(...)` entry as one `export interface`
    # declaration, in registration order. Returns "" when nothing is registered
    # so the heredoc collapses cleanly (no orphan blank line). Field types are
    # mapped through `crystal_to_ts` so the same generics-aware rules apply.
    private def self.render_registered_ts_types : String
      entries = Lune.registered_ts_types
      return "" if entries.empty?
      blocks = entries.map do |name, fields|
        body = fields.map { |fname, ftype| "  #{fname}: #{crystal_to_ts(ftype)};" }.join("\n")
