...
- **`Lune::Native::Tray.show` / `.hide` / click dispatch on Windows** — full tray-icon registration via `Shell_NotifyIconW(NIM_ADD/NIM_MODIFY/NIM_DELETE)` driven from a dedicated `Fiber::ExecutionContext::Isolated` "lune-tray" thread that owns a hidden message-only HWND (`HWND_MESSAGE` parent). The shell posts `WM_APP+1` to that HWND on every tray-icon click; the WindowProc inspects `lParam` and routes `WM_LBUTTONUP`/`WM_LBUTTONDBLCLK` to `on_tray_click` and `WM_RBUTTONUP` to `on_right_click`. Producer-side `show`/`hide` calls from any fiber dispatch through a Mutex-guarded ops queue with `Channel(Bool)` replies, same pattern as `Lune::Native::Hotkeys`. The message pump and op-queue drain share a single 10 ms tick loop. `LibUser32Tray` reuses `LibUser32Hotkeys::Msg` + `peek_message_w` to avoid Crystal's `fun`-redeclaration-with-divergent-types error on the shared `PeekMessageW` C symbol.
- **`Lune::Native::Tray.set_menu` / `.popup_menu` on Windows** — HMENU built on the pump thread via `CreatePopupMenu` + `AppendMenuW` (separator on `"---"`, `MF_STRING` otherwise) with sequential UInt32 command IDs mapped to user string IDs in an in-memory hash. `popup_menu` calls `SetForegroundWindow(hwnd)` (per MSDN, required for the menu to dismiss on outside click) → `TrackPopupMenu(TPM_RETURNCMD | TPM_RIGHTBUTTON)` at the current cursor position via `GetCursorPos` → looks up the returned command ID → invokes the menu callback. `PostMessageW(hwnd, WM_NULL, 0, 0)` after `TrackPopupMenu` works around the standard Win32 quirk where the next off-click doesn't dismiss cleanly. Re-entry from `WindowProc` (also on the pump fiber, since `DispatchMessageW` invokes it there) inlines `TrackPopupMenu` via a `Fiber.current == @@win32_pump_fiber` check — going through the op queue would self-deadlock since the pump can't drain new ops while a `WindowProc` call holds the fiber.
- **`Lune::Native::Tray.set_icon` on Windows** — custom `.ico` loading via `LoadImageW(NULL, path, IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_LOADFROMFILE)`. DPI-aware via `GetSystemMetrics`. Empty / missing / non-`.ico` paths fall back to the shared `LoadIconW(NULL, IDI_APPLICATION)` system icon with a `logger.warn` ("only .ico files are supported on Win32"). HICON lifecycle uses delayed destruction: the previous owned icon stays in `@@win32_pending_destroy` until after the next `Shell_NotifyIcon` call returns, then gets `DestroyIcon`'d, since Windows references the icon until the next `NIM_MODIFY` succeeds. `IDI_APPLICATION` is a shared system icon and is never `DestroyIcon`'d (`@@win32_h_icon_owned = false`). `Tray.show("path/to/icon.ico")` and `Tray.set_icon("path/to/icon.ico")` both honour the path; the bundled `assets/lune-logo.ico` is a multi-resolution PNG-encoded ICO (16/32/48/64/128/256 px) generated from `assets/lune-logo.png`.
- **`Tray.descriptor.platforms` includes `:win32`** — promoted from `[:darwin, :linux]` to `[:darwin, :linux, :win32]` now that all five tray bindings (`show`, `hide`, `set_icon`, `set_menu`, `popup_menu`) work on Windows. The capability now registers real bindings on Win32 instead of falling back to the platform-unavailable JS stub.

### Added

- **Per-capability platform gate** — `Descriptor` gained a `platforms : Array(Symbol)` field defaulting to `[:darwin, :linux, :win32]`. Capabilities whose list excludes the current OS are filtered out of the registry at construction time (resolved via compile-time `Lune::Capabilities::CURRENT_PLATFORM`), so they never get `setup`, never appear in the resolved set, and never participate in JS runtime generation. Three capabilities now declare narrower platform lists: `DragOut` (`[:darwin]`), `FileWatch` (`[:darwin, :linux]`), `FileDrop` (`[:darwin, :linux]`). (`Tray` was also `[:darwin, :linux]` at the start of this cycle but got promoted to all three once the Win32 implementation landed — see "Added — Windows port" above.) Previously their bindings either no-op'd silently (DragOut) or raised `NotImplementedError` on the unsupported platform — users had to know to add them to `capabilities.exclude` in `lune.yml` to silence the warnings. Now they're cleanly absent from the registry and emit rejecting JS stubs instead (see below).
