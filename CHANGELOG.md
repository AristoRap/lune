...
- **Three runtime behaviors are now disable-able via `lune.yml`** — `edit_shortcuts` (cmd/ctrl+A/C/V/X/Z/Y → execCommand), `navigation` (drives `opts.on_navigate`), `window_drag` (darwin-only, wires `opts.window_drag.zone`). Previously injected unconditionally from the runner; now first-class plugins you can opt out of via `plugins.disabled`.
