...
- **`Events` plugin renamed to `Event` (singular).** Aligns with the singular `Stream`. The class `Lune::Plugins::Events` → `Lune::Plugins::Event`, the bus class `Lune::Events` → `Lune::Event`, the app accessor `app.events.*` → `app.event.*`, the JS namespace `lune.Events.*` → `lune.Event.*`, the descriptor id and `lune.yml` config key `events` → `event`, and every `deps: [:events]` / `soft_deps: [:events]` symbol in dependent plugins (`context_menu`, `deep_link`, `file_drop`, `file_watch`, `tray`, `hotkeys`). Source files moved: `src/lune/bus/events.cr` → `bus/event.cr`, `src/lune/plugins/events.cr` → `plugins/event.cr`, spec and website pages mirror the rename. JS API is unchanged in shape — only the namespace identifier differs. Migration: `s/lune\.Events\./lune.Event./g` and `s/app\.events\./app.event./g` in user code; `events` → `event` in any `lune.yml` `enabled` / `disabled` lists; `Lune::Plugins::Events` → `Lune::Plugins::Event` if you typed the class directly. No deprecation shim — clean cut.
...
- **Bridge binding IDs unified across user and plugin surfaces.** User class `Demo.greet` and plugin `Tray.show` now produce bridge IDs of the same shape — `<Namespace>.<method>`. The `__lune.` prefix on plugin binding IDs is gone (was `__lune.tray.show`, is now `Tray.show`). The `__lune` global on `window` is unchanged — it's still the JS-side helper namespace (`window.__lune.on`, `crystalEmit`, …). Three previously hand-bound framework bindings now go through `@[Lune::Bind]` like every other plugin method: `__lune.jsEmit` → `Event.emit`, `__lune.startDrag` → `WindowDrag.start`, `__lune.navigate` → `Navigation.changed`. `wv.bind(…)` is no longer called from any plugin; every JS-callable Crystal method goes through `bind_deferred` via the macro. The `emit` JS helper on `runtime.Event` is now generated from the binding itself rather than hand-written in `js_helpers`. Sentinel namespace renamed from `"capabilities.__lune.<id>"` → `"lune.plugins.<id>"` (drops the doubled `__lune`).
- **Binding namespaces follow the Crystal module path 1-to-1.** A plugin at `Lune::Plugins::Tray` now produces the bridge ID `Lune.Plugins.Tray.show` (was `Tray.show`); the `Bindable` macro is identical for user and plugin classes — both interpolate `@type.name.stringify` and `Binding#id` splits `::` into `.`. Removes the collision risk where a user `class Tray < Lune::Bindable` and the built-in `Tray` plugin both produced `window["Tray.show"]`. JS runtime export shape changes to match: `runtime.js` exports a single `Lune` object (`Lune.Plugins.Tray.show()`) plus a short alias `export const lune = Lune.Plugins` so call sites can stay terse (`lune.Tray.show()`). The previous `runtime` aggregator and the per-plugin top-level exports (`export const Tray = …`) are gone. App-side `App.js` keeps `api` as an optional wrapper alongside the top-level user-class exports. Unavailable plugin stubs now return method-body fragments only; the generator wraps them in `{}` so they nest into the tree the same as live methods. The plugin's `binding_namespace` defaults to `self.class.name`; the redundant overrides on `Event`/`Stream`/`FileWatch` have been dropped. Generator collapses to one `build_tree` + JS / DTS renderers (~85 lines net reduction across the core).
...
- **Plugin config moves into each plugin class.** A `config do … end` macro on `Lune::Plugin` declares typed defaults that the plugin reads via `@config`, and reopens `Lune::Options` with an accessor named after the plugin's simple class name underscored. `Lune::Options::Tray` → `Lune::Plugins::Tray::Config`, `Lune::Options::FileDrop` → `Lune::Plugins::FileDrop::Config`, `Lune::Options::FileWatch` → `Lune::Plugins::FileWatch::Config`, `Lune::Options::Drag` → `Lune::Plugins::WindowDrag::Config` (also renames the accessor: `opts.drag` → `opts.window_drag`). Both `opts.tray { |t| t.event = … }` and `opts.tray.event = …` still work — they yield / return the registered plugin's `@config` instance via `Lune.registered_plugins`. `Lune::Options::Menu` and `Lune::Options::Mac` stay on `Options` because they're app-level, not plugin-scoped. `lune.yml`'s `plugins:` block stays registry-only (`enabled` / `disabled`); per-plugin config lives in the `Lune.run` block, which lets Procs flow without YAML hydration.
- **`ContextMenuBlocker` merged into `ContextMenu`.** The standalone plugin is gone; right-click suppression is now a config flag on `ContextMenu`: `opts.context_menu.block_default = true`. The top-level `opts.disable_context_menu` shortcut is also gone — one path for the feature instead of two. `lune.yml` migrators: drop `context_menu_blocker` from any `enabled` / `disabled` lists. Website: `/plugins/context-menu-blocker` removed; the `block_default` option is documented on `/plugins/context-menu`.
- **`opts.window_drag.value` removed.** Drag activation no longer matches a specific CSS value — any non-empty value on the configured property activates. The framework now checks `getPropertyValue(zone) !== ""` rather than `=== value`, so `style="--lune-draggable: true"` reads as a plain boolean flag. Demo's titlebar updated.
- **`WindowDrag` merged into `Window`.** The standalone `Lune::Plugins::WindowDrag` plugin is gone; drag-zone activation is now a flat property on the `Window` plugin's config: `opts.window.drag_zone = "--lune-draggable"` (was `opts.window_drag.zone = …`). The JS-side binding moves from `Lune.Plugins.WindowDrag.start` → `Lune.Plugins.Window.start_drag` and stays darwin-only via `{% if flag?(:darwin) %}` around the method def so the per-platform binding count is unchanged. Disabling drag specifically goes away — leaving `drag_zone` empty (the default) is the way to opt out; `plugins.disabled: [window]` turns off the whole plugin (including programmatic controls). Website: `/plugins/window-drag` deleted, content absorbed into `/plugins/window`. `lune.yml` migrators: drop `window_drag` from any `enabled` / `disabled` lists.
- **`Screen` merged into `System`.** The standalone `Lune::Plugins::Screen` plugin is gone; primary-screen info is now a method on the `System` plugin: `lune.System.screenInfo()` (was `lune.Screen.info()`), returning the same `{width, height, scale}` shape. Crystal: `System#screen_info` replaces `Screen#info`. The native shim at `Lune::Native::Screen.info` is unchanged — only the plugin layer collapsed. Website: `/plugins/screen` deleted, the `screenInfo` method documented on `/plugins/system`. `lune.yml` migrators: drop `screen` from any `enabled` / `disabled` lists.
- **`Notifications` merged into `System`.** The standalone `Lune::Plugins::Notifications` plugin is gone; native toasts are now a method on the `System` plugin: `lune.System.notify(title, body)` (was `lune.Notifications.notify(title, body)`), same `async: true` semantics so Win32's PowerShell shell-out keeps working. Crystal: `System#notify` replaces `Notifications#notify`. The native shim at `Lune::Native::Notifications.show` is unchanged. Website: `/plugins/notifications` deleted, the `notify` method documented on `/plugins/system` (platform-specific behaviour — `NSUserNotificationCenter` / `libnotify` / WinRT + AUMID — kept verbatim). `lune.yml` migrators: drop `notifications` from any `enabled` / `disabled` lists.
- **Menu shortcut parsing moved to the native boundary.** `Lune::Options::Mac::Shortcut` is gone; the parser that turned `"cmd+n"` into NSEvent flags + key chars now lives in ObjC at `ext/native/darwin/menu.m` alongside its only consumer, matching the way `Native::Hotkeys` already parses inside each platform's shim. `Options::Menu#to_json` emits the raw `shortcut: "cmd+n"` string instead of pre-parsed `key` / `modifiers` fields. Win32's menu builder ignores the field today (no menu-accelerator support yet); when added, it'll parse the string itself into Win32 flags rather than rely on macOS-shaped values baked into the JSON. User-facing `shortcut: "cmd+n"` API on menu items is unchanged. Internal dir rename to match: `ext/native/macos/` → `ext/native/darwin/` (mirrors `src/lune/native/darwin/`).
- **Hand-written TS interfaces dropped from `runtime.d.ts`.** `LuneEnvironment`, `ScreenInfo`, `TrayMenuItem`, and `ContextMenuItem` are no longer emitted as named `export interface` declarations alongside the runtime — the generator now derives every binding signature from the plugin source (Crystal `NamedTuple` auto-derives, `enum` auto-derives, anything else uses an inlined `@[Lune::BindOverride(ts_return_type: ...)]` literal). Same TS type information, no named carryovers. Migration for users who imported these names: alias the inferred return type at the call site, e.g. `type LuneEnvironment = Awaited<ReturnType<typeof lune.System.environment>>`. Only `LuneError` (a real runtime class, not a type-only declaration) stays in the header.
