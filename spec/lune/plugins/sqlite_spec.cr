require "../../spec_helper"

describe Lune::Plugins::Sqlite do
  describe "descriptor" do
    it "has correct id and label" do
      d = Lune::Plugins::Sqlite::DESCRIPTOR
      d.id.should eq(:sqlite)
      d.label.should eq("Sqlite")
    end

    it "has no hard deps" do
      Lune::Plugins::Sqlite::DESCRIPTOR.deps.should be_empty
    end

    it "is not core" do
      Lune::Plugins::Sqlite::DESCRIPTOR.core.should be_false
    end
  end

  describe "name and namespace" do
    it "derives name from descriptor" do
      Lune::Plugins::Sqlite.new.name.should eq("sqlite")
    end

    it "has Sqlite binding namespace" do
      Lune::Plugins::Sqlite.new.binding_namespace.should eq("Lune::Plugins::Sqlite")
    end
  end

  describe "phase membership" do
    it "includes Bindable" do
      Lune::Plugins::Sqlite.new.is_a?(Lune::Bindable).should be_true
    end

    it "includes Lifecycle" do
      Lune::Plugins::Sqlite.new.is_a?(Lune::Plugin::Lifecycle).should be_true
    end

    it "does not include WebviewInject" do
      Lune::Plugins::Sqlite.new.is_a?(Lune::Plugin::WebviewInject).should be_false
    end
  end

  describe "install" do
    it "registers open, close, exec, and query bindings" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      ids = app.bindings.map(&.id)
      ids.should contain("Lune.Plugins.Sqlite.open")
      ids.should contain("Lune.Plugins.Sqlite.close")
      ids.should contain("Lune.Plugins.Sqlite.exec")
      ids.should contain("Lune.Plugins.Sqlite.query")
    end

    it "open returns a 16-char hex db id for :memory:" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.open" }.not_nil!
      result = b.callback.call({"path" => JSON::Any.new(":memory:")}, nil)
      result.as_s.size.should eq(16)
    end

    it "close on a valid db_id returns nil" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      open_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.open" }.not_nil!
      close_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.close" }.not_nil!
      id = open_b.callback.call({"path" => JSON::Any.new(":memory:")}, nil).as_s
      result = close_b.callback.call({"db" => JSON::Any.new(id)}, nil)
      result.raw.should be_nil
    end

    it "close on unknown db_id does nothing" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      close_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.close" }.not_nil!
      result = close_b.callback.call({"db" => JSON::Any.new("nonexistent")}, nil)
      result.raw.should be_nil
    end

    it "exec creates a table and returns changes=0, lastInsertId=0" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      open_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.open" }.not_nil!
      exec_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.exec" }.not_nil!
      id = open_b.callback.call({"path" => JSON::Any.new(":memory:")}, nil).as_s
      result = exec_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("CREATE TABLE t (x INTEGER)"),
        "params" => JSON::Any.new([] of JSON::Any),
      }, nil)
      result["changes"].as_i64.should eq(0)
      result["lastInsertId"].as_i64.should eq(0)
    end

    it "exec insert returns rows_affected and lastInsertId" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      open_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.open" }.not_nil!
      exec_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.exec" }.not_nil!
      id = open_b.callback.call({"path" => JSON::Any.new(":memory:")}, nil).as_s
      exec_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("CREATE TABLE items (name TEXT, val INTEGER)"),
        "params" => JSON::Any.new([] of JSON::Any),
      }, nil)
      result = exec_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("INSERT INTO items VALUES (?, ?)"),
        "params" => JSON::Any.new([JSON::Any.new("hello"), JSON::Any.new(42_i64)]),
      }, nil)
      result["changes"].as_i64.should eq(1)
      result["lastInsertId"].as_i64.should eq(1)
    end

    it "exec raises sqlite_not_open for unknown db_id" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      exec_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.exec" }.not_nil!
      expect_raises(Lune::Error, "No open database") do
        exec_b.callback.call({
          "db"     => JSON::Any.new("bad"),
          "sql"    => JSON::Any.new("SELECT 1"),
          "params" => JSON::Any.new([] of JSON::Any),
        }, nil)
      end
    end

    it "exec raises sqlite_error for bad SQL" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      open_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.open" }.not_nil!
      exec_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.exec" }.not_nil!
      id = open_b.callback.call({"path" => JSON::Any.new(":memory:")}, nil).as_s
      err = expect_raises(Lune::Error) do
        exec_b.callback.call({
          "db"     => JSON::Any.new(id),
          "sql"    => JSON::Any.new("NOT VALID SQL !!!"),
          "params" => JSON::Any.new([] of JSON::Any),
        }, nil)
      end
      err.code.should eq("sqlite_error")
    end

    it "query returns rows as array of hashes" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      open_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.open" }.not_nil!
      exec_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.exec" }.not_nil!
      query_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.query" }.not_nil!
      id = open_b.callback.call({"path" => JSON::Any.new(":memory:")}, nil).as_s
      exec_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("CREATE TABLE people (name TEXT, age INTEGER)"),
        "params" => JSON::Any.new([] of JSON::Any),
      }, nil)
      exec_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("INSERT INTO people VALUES ('Alice', 30)"),
        "params" => JSON::Any.new([] of JSON::Any),
      }, nil)
      exec_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("INSERT INTO people VALUES ('Bob', 25)"),
        "params" => JSON::Any.new([] of JSON::Any),
      }, nil)
      rows = query_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("SELECT * FROM people ORDER BY age"),
        "params" => JSON::Any.new([] of JSON::Any),
      }, nil)
      arr = rows.as_a
      arr.size.should eq(2)
      arr[0]["name"].as_s.should eq("Bob")
      arr[0]["age"].as_i64.should eq(25)
      arr[1]["name"].as_s.should eq("Alice")
    end

    it "query with params uses ? placeholders" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      open_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.open" }.not_nil!
      exec_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.exec" }.not_nil!
      query_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.query" }.not_nil!
      id = open_b.callback.call({"path" => JSON::Any.new(":memory:")}, nil).as_s
      exec_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("CREATE TABLE nums (n INTEGER)"),
        "params" => JSON::Any.new([] of JSON::Any),
      }, nil)
      exec_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("INSERT INTO nums VALUES (1),(2),(3)"),
        "params" => JSON::Any.new([] of JSON::Any),
      }, nil)
      rows = query_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("SELECT n FROM nums WHERE n > ?"),
        "params" => JSON::Any.new([JSON::Any.new(1_i64)]),
      }, nil)
      rows.as_a.size.should eq(2)
    end

    it "query returns empty array for no rows" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      open_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.open" }.not_nil!
      exec_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.exec" }.not_nil!
      query_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.query" }.not_nil!
      id = open_b.callback.call({"path" => JSON::Any.new(":memory:")}, nil).as_s
      exec_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("CREATE TABLE empty_t (x TEXT)"),
        "params" => JSON::Any.new([] of JSON::Any),
      }, nil)
      rows = query_b.callback.call({
        "db"     => JSON::Any.new(id),
        "sql"    => JSON::Any.new("SELECT * FROM empty_t"),
        "params" => JSON::Any.new([] of JSON::Any),
      }, nil)
      rows.as_a.should be_empty
    end

    it "query raises sqlite_not_open for unknown db_id" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      query_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.query" }.not_nil!
      err = expect_raises(Lune::Error) do
        query_b.callback.call({
          "db"     => JSON::Any.new("bad"),
          "sql"    => JSON::Any.new("SELECT 1"),
          "params" => JSON::Any.new([] of JSON::Any),
        }, nil)
      end
      err.code.should eq("sqlite_not_open")
    end
  end

  describe "shutdown" do
    it "closes all open databases" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      open_b = app.bindings.find { |b| b.id == "Lune.Plugins.Sqlite.open" }.not_nil!
      open_b.callback.call({"path" => JSON::Any.new(":memory:")}, nil)
      open_b.callback.call({"path" => JSON::Any.new(":memory:")}, nil)
      plugin.shutdown
      # After shutdown, internal map is cleared — subsequent open works fine
      id = open_b.callback.call({"path" => JSON::Any.new(":memory:")}, nil).as_s
      id.size.should eq(16)
    end
  end

  describe "registry integration" do
    it "is included in the default resolved set" do
      r = Lune::Plugins::Registry.new(Pointer(Void).null, Lune::Options.new, -> { })
      resolved = r.resolve(Lune::Config::Plugins.new(enabled: nil, disabled: nil))
      resolved.plugins.map(&.name).should contain("sqlite")
    end

    it "can be excluded" do
      r = Lune::Plugins::Registry.new(Pointer(Void).null, Lune::Options.new, -> { })
      resolved = r.resolve(Lune::Config::Plugins.new(enabled: nil, disabled: ["sqlite"]))
      resolved.plugins.map(&.name).should_not contain("sqlite")
    end
  end

  describe "runtime.d.ts signatures" do
    it "emits open(path) as Promise<string>" do
      plugin = Lune::Plugins::Sqlite.new
      app = Lune::App.new
      app.install(plugin)
      dts = Lune::Generator.generate_runtime_dts(app.bindings, [plugin] of Lune::Plugin)
      dts.should contain("open(args: { path: string }): Promise<string>")
    end
  end
end
