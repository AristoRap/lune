...
describe Lune::RuntimeBindings do
  describe ".register" do
    it "does not pollute user binding_names" do
      ...
      quit_called = false
      Lune::RuntimeBindings.register(bridge, on_quit: -> { quit_called = true; nil })
      bridge.binding_names.should be_empty
    end
    ...
    it "invokes on_quit when __lune.quit is called" do
      ...
      quit_called = false
      Lune::RuntimeBindings.register(bridge, on_quit: -> { quit_called = true; nil })
      fake.invoke("__lune.quit", "seq-1", [] of JSON::Any)
    ...
    it "registers __lune.openURL and passes the url to on_open_url" do
      fake, bridge = make_bridge
      opened_url = ""
      Lune::RuntimeBindings.register(bridge, on_quit: -> { }, on_open_url: ->(url : String) { opened_url = url; nil })
      fake.invoke("__lune.openURL", "seq-2", [JSON::Any.new("https://example.com")])
    ...
    it "returns environment with os, arch, and debug fields" do
      fake, bridge = make_bridge
      Lune::RuntimeBindings.register(bridge, on_quit: -> { }, debug: true)
      fake.invoke("__lune.environment", "seq-3", [] of JSON::Any)
    ...
    it "reflects the debug flag in environment" do
      fake, bridge = make_bridge
      Lune::RuntimeBindings.register(bridge, on_quit: -> { }, debug: false)
      fake.invoke("__lune.environment", "seq-4", [] of JSON::Any)
    ...
    it "returns a known os value" do
      fake, bridge = make_bridge
      Lune::RuntimeBindings.register(bridge, on_quit: -> { })
      fake.invoke("__lune.environment", "seq-5", [] of JSON::Any)
      env = JSON.parse(fake.resolve_calls[0][2])
